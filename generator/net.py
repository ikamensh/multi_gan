...
def make_generator_model(noise_dim, num_classes, color_ch=3):
    ...
    cnn.add(layers.Dense(7*7*256, input_shape=(noise_dim,)))
    cnn.add(layers.BatchNormalization())
    cnn.add(layers.LeakyReLU())

    cnn.add(layers.Dense(7*7*128, input_shape=(noise_dim,)))
    cnn.add(layers.BatchNormalization())
    cnn.add(layers.LeakyReLU())
    ...
    cnn.add(layers.Conv2DTranspose(256, (1, 1), strides=(1, 1), padding='same', use_bias=False))
    assert cnn.output_shape == (None, 16, 16, 256), cnn.output_shape
    cnn.add(layers.BatchNormalization())
    cnn.add(layers.LeakyReLU())
    ...
    generator = tf.keras.Model([latent, image_class], corrected)
